...
class Location(ToDict):
    ...
    def is_local(self):
        return self.visibility == self.VISIBILITY_LOCAL
...
class RaceFinder(gcc.IpaPass):
    ...
    def analyze_statement(self, stat, context):
        #print '{}: {}'.format(str(context.fun.decl), str(stat))
        if 'CLOBBER' in str(stat):
            # fixed magic bug with segmantation
            return
    ...
    def rebind_summary(self, summary, args, context):
        ...
        summary.accesses.accesses = accesses
